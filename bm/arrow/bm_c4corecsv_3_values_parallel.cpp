    ...
    size_t num_threads = 4;
    {
        ...
        // read all the fields to the proper place
        {
            ...
            std::vector<std::thread> threads;
            {
                ...
                const size_t num_rows_per_thread = numlines / num_threads;
                for(size_t thread = 0; thread < num_threads; ++thread)
                {
                    threads.emplace_back([&, thread_id=thread, nrpt=num_rows_per_thread]{
                        TIMEDSECTION(thread_work);
                        csubstr my_data_lines = data_lines; // FIXME
                        const size_t rem = numlines % num_threads;
                        const size_t first_row = thread_id * nrpt + (thread_id != 0 ? rem : 0);
                        const size_t last_row = first_row + nrpt + (thread_id == 0 ? rem : 0);
                        for(size_t row = first_row; row < last_row; ++row)
                        {
                            csubstr line = my_data_lines.first(my_data_lines.first_of('\n'));
                            C4_ASSERT(line.len);
                            data.readline(line.trimr('\r'), row);
                            my_data_lines = my_data_lines.right_of(line.len + 1);
                        }
                    });
                }
            }
